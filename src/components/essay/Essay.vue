...
//每5s刷新数据
    timer = setInterval(() => {
        essay.value = essayList.value[index%essayList.value.length].content;
        index+=1;
    }, 1000);
})
...
function getEssay(){
    fetch(url+'/essay/get',{
        ...
        localStorage.setItem('essay',JSON.stringify(data.data));
    })
}
</script>
